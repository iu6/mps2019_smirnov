...
/*Функция считывания значения напряжения на АЦП и записи в память */
void write_track()
{
	uint32_t Address = 0;
...
void read_track(int norm)
{
	...
	for (i = 4; i < size_in_bytes; i++)
	{
		...
		if (norm == 1){
			...
			//Отбрасывание результатов, больших 0xFFF
...
void MY_ADC_1_Init(void)
{
	...
	ADC_1_Nastroyka.ADC_DelayGo = 2; //задержка между преобразованиями - 2 такта
...
{
	uint16_t result;
	while(MDR_ADC->ADC1_STATUS & 0x00000004 == 0){} //while conversation
...
int32_t main(void)
{
	...
	int current_track = 0; //current_track = 0,1,... - текущий трек
	int num_of_tracks = 1;
